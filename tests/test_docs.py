...
class DocumentationTests(unittest.TestCase):
    ...
    def test_docs_explain_first_run_app_entry_and_debug_export(self):
        ...
        for contents in (readme, docs_index, user_guide, release_readme):
            ...
            self.assertIn("Copy Repair Command", contents)
            self.assertIn("Copy Log Path", contents)
            self.assertIn("Export Diagnostics", contents)
            self.assertIn("app-group", contents)
            ...
            self.assertIn("Operation not permitted", contents)
    ...
    def test_docs_explain_first_transcript_flow_and_model_repair(self):
        ...
        for contents in (readme, docs_index, user_guide, release_readme):
            self.assertIn("Copy Repair Command", contents)
            self.assertNotIn("runs the non-interactive installer repair path", contents)
