...
class ReleaseDistributionTests(unittest.TestCase):
    ...
    def test_package_release_contains_required_distribution_payload(self):
        ...
        self.assertIn("docs/user-guide.md", script)
        self.assertIn("USER-GUIDE.md", script)
        self.assertIn("WebExtension", script)
        self.assertIn("SWinyDL-WebExtension.zip", script)
        self.assertIn("/usr/bin/zip", script)
        self.assertIn("Missing required runtime directory: %s", script)
        self.assertIn("REQUIRED_PAYLOAD_PATHS", script)
        self.assertIn("swinydl/main.py", script)
        self.assertIn("swinydl/version.py", script)
        self.assertIn("bin/parakeet-coreml-runner", script)
        self.assertIn("bin/speaker-diarizer-coreml-runner", script)
        self.assertIn("Release package is missing required payload", script)
        self.assertIn("pyproject.toml", script)
        self.assertIn("uv.lock", script)
        self.assertIn("install.sh", script)
        self.assertIn('chmod +x "$STAGE_ROOT/install.sh"', script)
