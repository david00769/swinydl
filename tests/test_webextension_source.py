...
class WebExtensionSourceTests(unittest.TestCase):
    ...
    def test_popup_exposes_app_selection_and_debug_controls(self):
        ...
        self.assertIn("response?.app_launch?.error", script)
        self.assertIn("Progress appears in SWinyDL", script)
        self.assertIn("If macOS asks SWinyDL to access data from other apps, click Allow.", script)
    ...
    def test_background_builds_sanitized_debug_export(self):
        ...
        self.assertIn("lastAppLaunchResult", contents)
        self.assertIn("lastQueuedSelection", contents)
        self.assertIn("selectedLessonAssetSummary", contents)
        self.assertIn("selected_lesson_asset_counts", contents)
        self.assertIn("recordNativeLaunchResponse", contents)
